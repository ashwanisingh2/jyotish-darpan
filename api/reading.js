...
function getFallbackResponse(name, type, lang) {
...
module.exports = async function handler(req, res) {
  ...
  try {
    body = req.body || {};
    const isStructured = body.type === 'daily' || body.type === 'kundali';
    ...
    if (isStructured) {
      ...
      try {
        ...
        return res.status(200).json(getFallbackResponse(body.name, body.type, body.lang));
      }
      
      const paragraphs = ['overview', 'career', 'love', 'health_remedies'];
      const parameters = ['lucky_color', 'lucky_number', 'lucky_time'];
      const missingParagraphs = paragraphs.filter(f => !parsed[f] || String(parsed[f]).trim().length < 5);
      const missingParameters = parameters.filter(f => !parsed[f] || String(parsed[f]).trim().length < 1);
      
      if (missingParagraphs.length > 0 || missingParameters.length > 0) {
        console.warn('Missing fields, falling back:', [...missingParagraphs, ...missingParameters]);
        return res.status(200).json(getFallbackResponse(body.name, body.type, body.lang));
      }
      
      const text = `🎯 Overview:\n${parsed.overview}\n\n💼 Career & Finance:\n${parsed.career}\n\n💑 Love & Relationships:\n${parsed.love}\n\n🌿 Health & Remedies:\n${parsed.health_remedies}\n\n🍀 Lucky Parameters:\nLucky Color: ${parsed.lucky_color} | Lucky Number: ${parsed.lucky_number} | Lucky Time: ${parsed.lucky_time}`;
    ...
    if (body && body.type) {
      return res.status(200).json(getFallbackResponse(body.name || 'Guest', body.type, body.lang));
    }
