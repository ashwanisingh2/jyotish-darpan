...
if(req.method==='GET'&&url.pathname==='/api/health')return send(res,200,{ok:true,aiConfigured:Boolean(API_KEY),version:'1.2.1'});
if(req.method==='GET'&&url.pathname==='/api/panchang'){const date=new Date(url.searchParams.get('date')||new Date().toISOString()),lat=url.searchParams.get('lat'),lon=url.searchParams.get('lon');if(!Number.isFinite(date.getTime())||!validCoordinates(lat,lon))return send(res,400,{error:'A valid date, latitude and longitude are required'});try{return send(res,200,panchang({date,lat:+lat,lon:+lon}))}catch{return send(res,400,{error:'Unable to calculate Panchang'})}}
if(req.method==='GET'&&url.pathname==='/api/geocode'){const q=cleanText(url.searchParams.get('q'),120);if(q.length<3)return send(res,400,{error:'Enter at least 3 characters'});try{const r=await fetch(`https://nominatim.openstreetmap.org/search?format=jsonv2&limit=5&q=${encodeURIComponent(q)}`,{headers:{'User-Agent':'JyotishDarpan/1.2 (contact via deployed website)'},signal:AbortSignal.timeout(8000)});if(!r.ok)throw new Error('Geocoder error');const d=await r.json();return send(res,200,d.map(x=>({name:x.display_name,lat:+x.lat,lon:+x.lon})))}catch{return send(res,502,{error:'Place search unavailable'})}}
if(req.method==='POST'&&url.pathname==='/api/kundali'){try{const b=JSON.parse(await readBody(req));if(!validDate(b.date)||(b.time&&!validTime(b.time))||!validCoordinates(b.lat,b.lon))return send(res,400,{error:'A valid date, time, latitude and longitude are required'});return send(res,200,kundali(b))}catch{return send(res,400,{error:'Unable to calculate chart'})}}
